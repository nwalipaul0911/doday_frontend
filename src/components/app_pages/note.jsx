...
const Note = () => {
  ...
  const [note, set_Note] = useState(loader_data)
  ...
  const saveNote = async () => {
    const response = await fetch(`http://127.0.0.1:9000/note/${note.id}`, {
      method: "PUT",
      headers: {
  ...
  return (
    <>
      ...
      <div className="backdrop" onClick={()=>navigate('/app/notes')}>
      
      </div>
      <div className={`py-4 bg-${theme} col-sm-10 col-md-6 px-4 rounded shadow mt-2 form-modal`}>
        <div className="d-flex justify-content-end"><i className={`fa-solid fa-xmark toggler ${text_color}`} onClick={()=>navigate('/app/notes')}></i></div>
